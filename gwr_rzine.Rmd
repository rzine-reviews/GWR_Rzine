...
```{r, echo=FALSE, fig.cap="Figure 2.8 du manuel INSEE [Codifier la structure de voisinage](https://www.insee.fr/fr/statistiques/fichier/3635442/imet131-f-chapitre-2.pdf) [@insee_2018]", out.width = '80%', fig.align = 'center'}
...
# (une classe sera créée autour de cette valeur, de taille 2*interval)
# min_nb : aucune classe ne contiendra moins de valeurs que min_nb
# (en regroupant les classes ayant un effectif trop faible)
discr <- function(values, center, pos_center, interval, min_nb) {
  ...
  x <- 1
  while (center + x * interval < max(values)) {
    # breaks <- append(breaks, center + x * interval)
    breaks <- append(breaks, centermax + x * interval)
    x <- x + 1
  }
  # ...pour les limites < centre
  x <- 1
  while (center - x * interval > min(values)) {
    # breaks = append(breaks, center - x * interval)
...
mf_map(x = data_immo, 
       ...
       leg_val_rnd = 1)
mf_title("Résidus de régression linéaire classique") #titre
...
<div class="alert alert-success" role="alert">
Le logiciel [GeoDa](http://geodacenter.github.io/), développé par Luc Anselin et son équipe pour étudier l'autocorrélation spatiale et les LISA, constitue une bonne solution logicielle en clic-boutons, avec une [documentation](http://geodacenter.github.io/documentation.html) riche [@geoda].
</div>
...
Pour réaliser les cartes, avec une discrétisation standardisée centrée sur la moyenne :

```{r}
par(mfrow = c(3,3))
for (var in colnames(data_immo)[22:29]) {
  # calcul des limites de classe
  res <- discr(data.frame(data_immo)[, var], mean(data.frame(data_immo)[, var]), "class_center", sd(data.frame(data_immo)[, var])*0.5, 10)
  breaks <- res[[1]]
  # palette de couleurs
  nb_cl_sup0 <- res[[2]]
  nb_cl_inf0 <- res[[3]]
