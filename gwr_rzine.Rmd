...
- Comme indiqué en introduction les données spatiales proviennent de la base [ADMIN-EXPRESS de l'IGN](https://geoservices.ign.fr/adminexpress){target="_blank"} en accès libre. La couche est utilisée est celle des EPCI de la base ADMIN-EXPRESS-COG édition 2022 par territoire pour la France métropolitaine.
...
<hr>
Sur R on peut facilement réaliser cette opération avec la fonction `scale()`. La standardisation peut aussi se faire "manuellement" : il suffit de soustraire chaque valeur par la moyenne puis de diviser par l'écart-type.</div>

## Chargement des données géographiques : les EPCI de France métropolitaine

Ces données proviennent de la base [ADMIN-EPXRESS-COG de l'IGN](https://geoservices.ign.fr/adminexpress){target="_blank"}, édition 2022. Le format d'entrée est le shapefile que nous allons lire dans un objet sf :
...
# on voit qu'il a pour voisins les poygones 62, 74 etc.
neighbours_epci[[1]]
...
<div class="alert alert-danger" role="alert">
Ainsi, si on s'intéresse au lien entre les voisins on est dans l'autocorrélation spatiale et les modèles SEM, SAR & cie mais si on s'intéresse à l'hétérogénéité des variables c'est-à-dire à leur variabilité selon leur localisation on est dans la GWR.
</div>
...
Pour visualiser la non stationnarité des effets des VI la solution la plus efficace est la carte.
